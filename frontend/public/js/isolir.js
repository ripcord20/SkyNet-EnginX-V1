...
window.deleteDevice = async function(id) {
  ...
  const prev = _devices.slice();
  _devices = _devices.filter(x => parseInt(x.id) !== parseInt(id));
  const el = document.getElementById('deviceList');
  if (el) {
    const btn = el.querySelector(`button.dev-act.danger[onclick="deleteDevice(${id})"]`);
    const card = btn && btn.closest('.dev-card');
    if (card) card.remove();
  }
